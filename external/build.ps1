...
$d = "oneTBB"
foreach ($c in $config) {
    cmake -S $d -B $d/build -DCMAKE_INSTALL_PREFIX="install/$c" -DBUILD_SHARED_LIBS=ON -DTBB_TEST=OFF "-DCMAKE_BUILD_TYPE:string=$c" -G "Ninja" $CC $CXX 
...
$d = "googletest"
foreach ($c in $config) {
    cmake -S $d -B $d/build -DCMAKE_INSTALL_PREFIX="install/$c" -DBUILD_SHARED_LIBS=ON "-DCMAKE_BUILD_TYPE:string=$c" -G "Ninja" $CC $CXX
    cmake --build $d/build --config $c --target install -j 20
}
Remove-Item $d/build -Force -Recurse
...
$d = "benchmark"
foreach ($c in $config) {
    cmake -S $d -B $d/build -DCMAKE_INSTALL_PREFIX="install/$c" -DBENCHMARK_ENABLE_TESTING=OFF "-DCMAKE_BUILD_TYPE:string=$c" -G "Ninja" $CC $CXX
    cmake --build $d/build --config $c --target install -j 20
}
Remove-Item $d/build -Force -Recurse